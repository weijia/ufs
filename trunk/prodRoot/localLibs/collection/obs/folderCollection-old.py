...
class folderStorageStateOnDb:
    ...
    def updated(self, item):
        ncl(item.itemInfo)
        return self.collection.updated(item.getItemObjUrl())
    def update(self, item):
        ...
        ncl(remoteInfo)
        ncl(itemUuid)
        self.collection.addVirtualObj(item.getItemObjUrl(), itemUuid)
        
    def updateLocal(self, item):
        self.collection.updateObj(item.getItemObjUrl())
        
    def getItemState(self, item):
        ncl(item.getItemObjUrl())
        res = self.collection.getObj(item.getItemObjUrl())
        ncl("returnning", res)
        return res
