...
class extractedZipStorageItem(objectDatabase.objInCollection):
    def __init__(self, rootPath, fullPath, itemInfo, zipFileFullPath, zipFileObj, pathInZipFile):
        self.rootPath = transform.transformDirToInternal(rootPath)
        objectDatabase.objInCollection.__init__(self, pathInZipFile, itemInfo)
        self.itemInfo["zipFileFullPath"] = zipFileFullPath
        self.itemInfo["pathInzipFile"] = pathInZipFile
    ...
    def getItemInfo(self):
        info = copy.copy(self.itemInfo)
        info["originalUuid"] = info["uuid"]
        del info["uuid"]
        return info
        
    def saveTo(self, targetRootPath):
        print 'saving to:', transform.transformDirToInternal(os.path.join(targetRootPath, 
                                                                          self.pathInZipFile)).encode('gbk','replace')
        self.zipFileObj.extract(self.pathInZipFile, targetRootPath)

    def getIdInCol(self):
        return transform.formatRelativePath(self.pathInZipFile)

class encZipInfoCollectionEnum(synchronizableCollection.synchronizableCollection):
