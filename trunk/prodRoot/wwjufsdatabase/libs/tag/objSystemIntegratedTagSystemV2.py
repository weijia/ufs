...
class tagSystemShoveDb(inheritableExclusableTagSystem.tagSystemShoveDb):
    ...
    def getTags(self, full_path):
        self.addToObjSysForFullPath(full_path)
        #print url
        res = inheritableExclusableTagSystem.tagSystemShoveDb.getTags(self, full_path)
        #print res
        pathTag = []
        for i in self.objSys.getSameObjsForFullPath(full_path):
