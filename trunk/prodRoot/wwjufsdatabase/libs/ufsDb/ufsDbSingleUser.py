...
try:
    ...
    import localLibs.localDb.mongodb.mongoShoveForSessionV3 as shove
...
class dbSysSmart:
    ...
    def getDb(self, dbName):
        db = shove.ShoveForSession(dbName, self.sessInst, self.connection)
        #return db
        import libs.shove.encryptedListShove as encShove
        return encShove.ShoveLike(db, simpleKey = self.sessInst.getPasswd())
    def getDbNameList(self):
        return shove.getDbList()
    def getTimeStamp(self):
        return shove.getSnapshotTimestamp()
    def importDb(self, objList):
        return shove.importDb(objList)
    def exportDb(self, fromTime = None, toTime = None, exporter = 'home-ibm'):
        return shove.exportDb(fromTime, toTime, exporter)