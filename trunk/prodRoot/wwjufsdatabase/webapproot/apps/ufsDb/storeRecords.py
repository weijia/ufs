...
for qs in qsl:
  ...
  for ql in qll:
    if ql.split('?')[0] == 'add':
      ...
      objId = urllib.unquote(p[1].split('=')[1])
      key = urllib.unquote(p[2].split('=')[1])
      value = urllib.unquote(p[3].split('=')[1])
      createapp = urllib.unquote(p[4].split('=')[1])
      if not dbHash.has_key(dbName):
        dbHash[dbName] = mngr.getSimpleDb(u, dbName)
      dbHash[dbName].add(objId, key, value, createapp)