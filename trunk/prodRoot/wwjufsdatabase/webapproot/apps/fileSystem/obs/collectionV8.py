...
def showThumbCollection(iter, divClassName = "thumbDiv", imgClass="thumbImage"):
    ...
    for i, thumbPath in iter:
        ...
        tagListUtf8 = []
        for k in tList:
            tagListUtf8.append(k.decode('utf8'))
        tagStr = ','.join(tagListUtf8)
        '''
        ...
        if tagStr == u"":
            tagStr = u"input tag"
        if i != thumbPath:
            res += u'<img class="%s" src="%s%s" width="100px" height="100px" path="%s"/><p class="tagEditor">%s</p>'%(imgClass, thumbViewScript, 
                thumbPath,i, tagStr)
        else:
            res += u'<img class="%s" src="%s%s" width="100px" height="100px" path="%s"/><p class="tagEditor">%s</p>'%(imgClass, 
                thumbViewScript, thumbPath, i, tagStr)
        res += '</div>\n'
    return res
