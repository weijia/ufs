...
import webapproot.apps.localServer.collectionWithPreviewV3
...
def showDirTree():
    ...
    h.script('''$(function (){$("#%s").bind("select_node.jstree", function(e, data) {
        ...
        $("#collectionList").autoPager("reload", "collectionV4.py?path="+path.replace("\\\\","\\\\\\\\")+"&page=");
    });});
    '''%"dirTreeContainer")
...
if __name__=='__main__':
    ...
    webapproot.apps.localServer.collectionWithPreviewV3.collectionWithPreview(h, 'C:/',None)
    h.write('</div>')
    h.genEnd()
