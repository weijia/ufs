...
class archiverInterface:
    ...
    def archive(self, archivableInst, archiveStorageInst):
        ...
        index = 0
        processLen = 50
        #Indicate if the current directory was updated, if it is not, 
        #do not check the files in current dir? No, as only when files were added
        #or deleted, the modify time will changed. If a file was modified, the
        #time of its parent directory will not change.
        ...
        while True:
            ...
            for i in res:
                ...
                if archivableInst.child(i).isContainer():
                    ...
                    if archiveStorageInst.updated(archivableInst.child(i)):
                        archiveStorageInst.store(archivableInst.child(i))
                        archiveStorageInst.updateModTime(archivableInst.child(i))
        archiveStorageInst.updateModTime(archivableInst)
